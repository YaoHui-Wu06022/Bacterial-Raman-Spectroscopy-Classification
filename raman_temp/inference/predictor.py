...
from raman_temp.core.hierarchy import normalize_level_name, resolve_allowed_indices
from raman_temp.core.hierarchy_meta import load_hierarchy_meta
from raman_temp.core.input_spec import InputSpec, build_input_spec
from raman_temp.modeling.factory import build_model, validate_model_input
...
class Predictor:
    ...
    def predict_tensor(
        ...
        for level_name in self.level_order:
            entry, parent_id = _resolve_level_entry(self.meta, level_name, parent_id)
...
def _resolve_input_entry(
    ...
    for current_level in meta.get("head_names") or []:
        entry, parent_id = _resolve_level_entry(meta, current_level, parent_id)
        if current_level == level_name:
            return entry
    raise RuntimeError(f"未找到层级模型：{level_name}")
