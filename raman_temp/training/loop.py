...
from raman_temp.training.spec import ExecutionSpec, TrainingSpec
from raman_temp.training.checkpoint import (
    TrainingState,
...
from raman_temp.training.optimizer import build_optimizer, build_scheduler
from raman_temp.training.split import TrainTask
    ...
    try:
        for epoch in range(start_epoch, train_spec.epochs + 1):
            ...
            if train_state.patience_counter >= train_spec.patience:
                log_message("EarlyStopping triggered by validation score")
                break
    except Exception:
        raise
    else:
...
def _run_train_epoch(
    ...
    for batch_index, (inputs, labels, sample_paths) in enumerate(train_loader):
...
def _update_ema_class_loss(
    ...
    for class_index in range(train_task.num_classes):
        ...
        train_state.ema_class_ce[class_index] = (
            ...
            * train_state.ema_class_ce[class_index]
            + (1.0 - train_context.train_spec.loss.ema_alpha) * class_loss
        )
...
def _update_best_model(
    ...
    if score > train_state.best_score:
        ...
        log_message("Best model updated")
    else:
