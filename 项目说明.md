...
├─ cli.py                            # 离线数据处理 CLI：pack/unpack/train/plot/count
...
├─ stage.py                          # invalid / similar 两阶段判定规则
...
4. 使用 `raman.audit full --stage ...` 对 `init/` 中的原始库做分阶段扫描
5. 结合审核报告和候选图人工确认异常谱
...
9. 独立测试集直接放入或复制到 `test/`，后续由 infer 读取
...
`--delta` 的单位是 `cm^-1`，正值向右平移，负值向左平移

当前累计平移量记录在 `fig_init/delta.txt`；每次平移动作会追加到 `fig_init/delta_log.txt`，包括单次增量、累计增量、修改文件数和可选备注
...
```bash
python -m raman.audit full <数据集名或profile> --stage invalid
python -m raman.audit full <数据集名或profile> --stage invalid --move

python -m raman.audit full <数据集名或profile> --stage similar
python -m raman.audit full <数据集名或profile> --stage similar --move
```
...
具体阈值统一维护在 `raman/audit/config.py` 的 `AuditConfig` 中

当前 audit 不再维护复核候选阈值，也不自动生成复核候选清单。输出只代表达到删除线的强候选，后续是否移动仍由人工审图后决定
...
dataset/<数据集名>/delete/Similar Outliers/
```

也可以先 dry-run，再用 `move` 手动执行：

```bash
python -m raman.audit move <数据集名或profile> --from-list <时间戳>_<stage>_dry_run --dry-run
python -m raman.audit move <数据集名或profile> --from-list <时间戳>_<stage>_dry_run
python -m raman.audit move <数据集名或profile> Burkholderia/BCC01/CELL8_Area01_000_shift.arc_data --reason "Similar Outliers" --category "Similar Outliers" --dry-run
```
...
- 默认不覆盖已有文件
- `--from-list` 可以传完整 CSV 路径，也可以只传 `audit_full_scan` 下的输出目录名，程序会自动读取其中的 `delete_candidates.csv`
...
独立测试谱直接放入或复制到 `dataset/<数据集>/test/`

infer 只读取 `test/`，不会读取 `train/`，也不会临时构建测试集

只有开启训练均值谱对照时，才会额外读取 `train/` 生成均值线

推理入口统一放在 `raman.infer`：

```powershell
python -m raman.infer test --exp-dir " " --level level_1
```
...
输出目录按当前选择的模型 run 落在 `test_result/` 下
