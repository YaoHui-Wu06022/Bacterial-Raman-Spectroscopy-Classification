...
class AuditConfig:
    ...
    class_corr_ref_remove_max: float = 0.82  # 与同前缀中位谱相关性低于此值，作为删除强证据
    class_rmse_ref_remove_min: float = 0.75  # 与同前缀中位谱 RMSE 高于此值，作为删除强证据
    class_local_residual_min: float = 0.25  # 局部正残差异常连续区域的残差下限
    class_local_residual_width_min_points: int = 12  # 局部残差异常最小连续点数
    class_local_residual_remove_area_min: float = 40.0  # 局部正残差超出阈值的面积达到此值，作为删除强证据
