...
├─ config_io.py                      # shared/model/resolved 配置读写与实验配置回载
...
├─ build.py                          # 离线构建主流程：从 init 生成 train
...
```text
raman/audit/
├─ cli.py                            # 连续清洗、回滚平移与审图 CLI
├─ workflow.py                       # Stage1/自动平移/Stage2/Stage3 主流程
├─ test_pool.py                      # 测试菌临时审核池的复制、回写和 *t 重建
├─ plots.py                          # 同前缀预览与单文件夹平移对照图
├─ common.py                         # 审核侧预处理共用逻辑
├─ genus_review.py                   # 按属复核批次稳定性与跨属边界
├─ pair_review.py                    # 000/001 重复测量对复核
...
raman/
├─ experiment.py                     # 实验根、run、模型与结果目录解析
├─ runtime.py                        # 模型懒加载、父类路由与级联推理
...
- `init.npz`：`init/` 的打包版本
- `train/`：训练集离线清洗结果
- `init_test/`：独立测试原始来源，`raman.data test` 从这里重建 `test/`
- `test/`：预处理后的独立测试集；一般 profile 的 infer 默认读取这里
- `fig_init/`：`raman.audit preview`、`plot-shift` 输出和 delta 审图状态
- `fig_train/`：训练集均值谱图与高层级均值谱图
- `audit_runs/`：连续审核的分阶段报告、候选表和复核图
- `delete/`：连续审核移出的原始 `.arc_data` 文件，按 `stage1`、`stage2`、`stage3` 分类
...
1. 准备或还原原始训练数据到 `init/`；独立测试原始数据放入 `init_test/`。
2. 对 `cos` / `test` 联合库，先执行 `python -X utf8 -m raman.audit clean`。它会同步测试池、执行三阶段审核、自动平移并重建训练用 `*t` 副本。
3. 用 `python -X utf8 -m raman.audit preview --dataset cos` 查看同属同前缀中位谱；对已自动平移的文件夹，用 `plot-shift` 复核。
4. 用 `python -X utf8 -m raman.data train <profile>` 重建 `train/`；按需用 `plot` 和 `count` 审图、计数。
5. 用 `python -X utf8 -m raman.data test <profile>` 从 `init_test/` 重建 `test/`；或者由 `infer_test.py` 的 `BUILD_TEST_FROM_INIT` 为测试菌重建。

`raman.data train` 会重建 `train/` 并清空后重建 `fig_train/`，`raman.data test` 会重建 `test/`；这些目录只应保存可再生结果。

### 4.5 原始谱与平移审核

当前不再提供 `raman.shift` 或手工 `apply` 命令。对 `cos` 数据集，自动平移是连续审核的一部分：Stage 1 后，代码在 1002 cm⁻¹ 锚点附近估计各文件夹的峰位，并在满足质量、残余范围和累计平移限制时写回波数列。当前累计量保存在数据集根目录的 `delta.txt`，操作历史保存在 `delta_log.txt`。

```bash
python -X utf8 -m raman.audit clean
python -X utf8 -m raman.audit preview --dataset cos
python -X utf8 -m raman.audit plot-shift --dataset cos --folder <Genus/Folder>
python -X utf8 -m raman.audit rollback-shift --dataset cos
```

`preview` 按 `delta.txt` 的变更增量生成同属同前缀的 raw、SNV 和 minmax 中位谱图，默认包含 `*t` 迁移目录。`plot-shift` 为已记录累计平移的文件夹输出平移前后 raw 中位谱及同前缀参考曲线。`rollback-shift` 只撤销当前 `delta.txt` 所代表的累计平移。

### 4.6 原始库审核与人工移除

当前审核入口是 `python -X utf8 -m raman.audit clean`，而不是旧的 `full` / `move` 子命令。成功运行时，它按 Stage 1（原始坏谱）→ 自动平移 → Stage 2（同属同前缀全量近邻）→ Stage 3（单批次近邻）顺序执行。强候选会在各阶段直接移动到 `delete/stage1`、`delete/stage2` 或 `delete/stage3`；`local_review` 仅保留在报告中。

因此，`clean` 是会改写数据的操作。它还会把测试菌 CS 谱复制到临时审核池，审核后回写保留谱，并重建 `*t` 训练副本和迁移清单。运行前应先确认 `delete/` 可作为可恢复归档。

#### 三阶段审核与相似性原理

下面保留相似性审核的统计思想、相关系数和 RMSE 推导，便于理解为什么近邻比较能够识别异常谱；当前可执行实现以 `raman/audit/workflow.py` 为准。它不再使用旧版 `invalid` / `similar` 命令、`AuditConfig` 或人工执行的候选移动清单。
...
- `invalid_flat_region`：连续平坦区过长、长段无信息，或全谱过多点贴近中位数
- `invalid_noise`：高频细节波动过大，谱线锯齿或抖动明显
...
- `local_bump_*`：目标谱相对参考中位谱的局部 SNV 正残差凸起

另外会输出 `folder_candidate_count` 和 `folder_candidate_fraction`，用于人工审图时观察同一小文件夹内候选是否过度集中。这两个字段只用于辅助判断，不参与当前自动删除判定
...
Pearson 相关系数也可以理解为“中心化后的余弦相似度”

目标谱经过 SNV 后均值理论上已经接近 0，但逐波数中位数组成的参考谱均值不一定为 0，因此计算时仍会对两条谱分别减去自身均值。相关系数越接近 1，说明峰位、峰形和相对峰高的整体变化越一致

当前参考中位谱由同属同前缀的全部有效谱构建，包含目标谱自身，参考池较大时，单条谱通常很难明显改变逐点中位数
...
- `corr_ref` 过低：整体峰形、峰位和相对峰高与同前缀参考谱不够同步
- `rmse_to_ref` 过高：即使整体走势相似，逐点幅度差异仍然过大
- `local_bump_area` 过高：整体仍然相似，但相对参考中位谱多出一段连续正向凸起。这个指标用平滑后的 `sample_snv - reference_median` 残差计算，只积分超过残差阈值线的超额面积，更接近审图时看到的“某一段峰高整体偏高”

`corr_ref` 和 `rmse_to_ref` 需要同时达到删除阈值，才形成整体相似性删除倾向。局部凸起单独达到删除面积线时，也会形成删除倾向

当前阈值与判定逻辑定义在 `raman/audit/workflow.py`。Stage 2 在同属同前缀的全量池中寻找近邻；Stage 3 只在单一采集批次中寻找近邻。两者均使用近邻相关性、近邻中位参考谱的 RMSE 与局部残差面积；强候选由 `clean` 直接移动，局部复核项仅写入报告。

#### 审核输出与移动

每个 stage 的 CSV、候选图和 `run.json` 写入 `dataset/50种菌cos/audit_runs/<timestamp>_<stage>/`。移动目标固定为 `delete/stage1`、`delete/stage2` 或 `delete/stage3`，不会覆盖已存在的同名目标。旧的 `raman.audit move`、`--dry-run`、`--from-list` 和 `audit_full_scan/` 路径均不再适用。
...
调度器直接作用在该 optimizer 上

#### 分组学习率

当前实现不再按模块划分学习率；下面关于分组学习率的原理说明保留作背景，实际训练以单一学习率组运行。
...
| ---------- | ------------- | ------------ | ---------------- | ------------ |
| 三分类     | 第 2 名       | `1.00`       | `1.10`           | `0.88`       |
| 三分类     | 第 3 名       | `1.08`       | `1.20`           | `0.88`       |
| 四类及以上 | 第 2 名       | `1.00`       | `1.10`           | `0.85`       |
| 四类及以上 | 第 3 名       | `1.05`       | `1.20`           | `0.85`       |
| 四类及以上 | 第 4 名及以后 | `1.10`       | `1.25`           | `0.85`       |
...
通过 `EVAL_MODE` 显式选择三种模式：

- `single_model`：传入具体 `run_*` 或 `best/run_*` 目录，只评估这一个模型
- `level_only`：传入实验根目录，只使用目标层模型，按真实父类分发 parent 子模型
- `cascade`：传入实验根目录，从顶层到目标层按预测结果逐层级联
...
一般 profile 的独立测试谱先经 `python -X utf8 -m raman.data test <profile>` 从 `init_test/` 重建为 `dataset/<数据集>/test/`。根目录 `infer_test.py` 也可以在 `BUILD_TEST_FROM_INIT=True` 时，从 `dataset/测试菌/init` 重建 `dataset/测试菌/test`。

infer 不读取 `train/` 作为测试输入；只有开启训练均值谱对照时，才会额外读取 `train/` 生成均值线。以 `50种菌cos` 训练的模型在未显式提供 `test_root` 时，推理核心会默认使用 `dataset/测试菌/init`，这是该 profile 的特例。

推理入口统一放在 `raman.infer`：

```powershell
python -X utf8 -m raman.infer test --exp-dir output/<dataset>/<experiment> --level level_1
```
...
`infer_test.py` 会从实验配置读取输入参数；默认配置会先重建测试菌的 `test/`，再将其作为 `TEST_ROOT` 传给推理。
