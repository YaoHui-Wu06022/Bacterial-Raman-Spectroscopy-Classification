...
# level_2用
# config.align_start = 20
# config.align_end = 35
# config.supcon_start = 25
# config.supcon_end = 45

# 可选：覆盖损失参数（单独训练时可能不同）
OVERRIDE_ALIGN_LOSS_WEIGHT = None  # level1 0.02 level2 0.015 level3 0.01/0
OVERRIDE_SUPCON_TAU = None
OVERRIDE_SUPCON_LOSS_WEIGHT = None # level1 0.05 level2 0.03 level3 0
...
def main():
    ...
    def train_single_model(
        ...
        for epoch in range(1, config.epochs + 1):
            ...
            # 对齐 / SupCon 后期衰减
            decay_start = int(0.6 * config.epochs)
            decay_ratio = 1.0
