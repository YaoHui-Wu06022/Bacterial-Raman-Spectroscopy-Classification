...
默认 preview 会忽略以 `t` 结尾的迁移目录，例如 `KP06t`

需要把这些插入训练集的独立测试来源目录也纳入对照时，运行：
...
计算前，每条原始谱都会临时执行当前离线预处理流程，包括宇宙射线去除、基线校正、裁剪、坏段剔除和统一波数轴插值，随后对单条谱做 SNV。设目标 SNV 谱为

$$
x=(x_1,\dots,x_L)
$$

同属同前缀参考池在每个波数点分别取中位数，得到参考中位谱

$$
m_i=\mathrm{median}\left(x_i^{(1)},x_i^{(2)},\dots,x_i^{(N)}\right)
$$

当前 `corr_ref` 使用目标谱与参考中位谱之间的 Pearson 相关系数：

$$
\mathrm{corr\_ref}
=
\frac{
\sum_{i=1}^{L}(x_i-\bar{x})(m_i-\bar{m})
}{
\sqrt{\sum_{i=1}^{L}(x_i-\bar{x})^2}
\sqrt{\sum_{i=1}^{L}(m_i-\bar{m})^2}
}
$$

Pearson 相关系数也可以理解为“中心化后的余弦相似度”。目标谱经过 SNV 后均值理论上已经接近 0，但逐波数中位数组成的参考谱均值不一定为 0，因此计算时仍会对两条谱分别减去自身均值。相关系数越接近 1，说明峰位、峰形和相对峰高的整体变化越一致；它不直接衡量原始绝对强度是否相同。

当前参考中位谱由同属同前缀的全部有效谱构建，包含目标谱自身。参考池较大时，单条谱通常很难明显改变逐点中位数。`nearest_ref_corr` 使用相同的 Pearson 公式，但会排除目标谱所在的小文件夹，再从其它同前缀小文件夹中寻找相关系数最高的单谱：

$$
\mathrm{nearest\_ref\_corr}(x)
=
\max_{y\in\mathcal{R}_{\mathrm{otherFolder}}}
\mathrm{corr}(x,y)
$$

`rmse_to_ref` 则直接衡量目标 SNV 谱与参考中位谱之间的点对点距离：

$$
\mathrm{rmse\_to\_ref}
=
\sqrt{
\frac{1}{L}
\sum_{i=1}^{L}(x_i-m_i)^2
}
$$

相关系数更关注两条谱的整体形状是否同步，RMSE 更关注对应波数点上的实际差异幅度，因此两者并不重复。例如两条谱的峰位和整体走势相似，但部分峰高明显不同，可能仍有较高相关系数，同时产生较高 RMSE。

局部正残差使用目标谱减去参考中位谱，并除以参考池在每个波数点上的鲁棒尺度。连续正残差区域达到设定宽度、峰值 z 和面积后，会形成 `local_pos_*` 证据，用于发现“整体仍然相似，但局部存在异常宽凸高”的谱。

整体相关性判定中，`corr_ref` 过低、`nearest_ref_corr` 过低和 `rmse_to_ref` 过高分别算一项证据。单项达到强阈值通常不会直接触发删除；至少两项整体证据达到强阈值，或者局部正残差单独达到强异常条件，才形成删除倾向。具体阈值统一维护在 `raman/audit/config.py` 的 `AuditConfig` 中。
...
- `baseline`：残余背景扰动
  ...
  **弱 baseline 扰动**：模拟幅度较小、形状较平滑的背景残留
  ...
  弱扰动背景为 $b_{\mathrm{weak}}(i) = \alpha i + \beta \sin(2\pi f i + \phi)$
  ...
  **强 baseline 扰动**：模拟更明显的批次差异或仪器背景漂移
  ...
  强扰动背景为 $b_{\mathrm{strong}}(i) = \mathrm{Interp}((i_1,y_1),\dots,(i_m,y_m))$
...
$$
x \rightarrow \mathrm{Conv1d}(\mathrm{kernel}=1) \rightarrow \mathrm{Conv1d}(\mathrm{kernel}=3) \rightarrow \mathrm{Conv1d}(\mathrm{kernel}=1) \rightarrow \mathrm{SE} \rightarrow +\;\mathrm{shortcut} \rightarrow \phi
$$
...
$$
\mathrm{mid\_channels} = \max \left(\left\lfloor \frac{\mathrm{out\_channels}}{\mathrm{bottleneck\_ratio}} \right\rfloor, 1\right)
$$
...
$$
\mathrm{mid\_channels} =
\max\left(
\mathrm{out\_channels} \cdot \frac{\mathrm{base\_width}}{64} \cdot \mathrm{cardinality},
\mathrm{cardinality}
\right)
$$
...
$$
\mathrm{seq\_dim} = 2 \times \mathrm{lstm\_hidden}
$$

否则就是:

$$
\mathrm{seq\_dim} = \mathrm{lstm\_hidden}
$$
...
- $w_{y_n}(t)$ 是真实类别 $y_n$ 在当前训练轮次 $t$ 使用的类别权重
- $w_{y_n}(t)$ 不属于 Focal Loss 的调制因子，而是本项目在 Focal Loss 外叠加的类别重加权
...
当前实现另外接收 `weight`，并按真实类别取出 $w_{y_n}(t)$，得到本项目实际使用的逐样本分类损失：

$$
L_{\mathrm{cls\_each}}^{(n)}(t)
=
w_{y_n}(t)(1-p_t^{(n)})^\gamma
\left[-\log(p_t^{(n)})\right]
$$
...
| 类别数 | 真实类别排名 | 常规样本权重 | 高置信度错判权重 | 高置信度阈值 |
|---|---:|---:|---:|---:|
| 二分类 | 任意 | `1.00` | `1.00` | 不启用 |
| 三分类 | 第 2 名 | `0.95` | `1.05` | `0.88` |
| 三分类 | 第 3 名 | `1.10` | `1.25` | `0.88` |
| 四类及以上 | 第 2 名 | `0.90` | `1.10` | `0.85` |
| 四类及以上 | 第 3 名 | `1.00` | `1.35` | `0.85` |
| 四类及以上 | 第 4 名及以后 | `1.10` | `1.35` | `0.85` |

二分类不额外使用 `severity weight`
...
3. 按下式计算基础权重
   ...
   $$
   \mathrm{base\_class\_weight}_c = \frac{1}{\log(\mathrm{count}_c + 1.5)}
   $$

4. 再归一化到平均值为 1

   $$
   \mathrm{base\_class\_weight}_c \leftarrow \frac{\mathrm{base\_class\_weight}_c}{\frac{1}{K} \sum_{j=1}^{K} \mathrm{base\_class\_weight}_j}
   $$
...
$$
\mathrm{center}_c^{(\mathrm{batch})}= \frac{1}{|S_c|}
\sum_{n \in S_c}  x_n
$$
...
$$
L_c^{(\mathrm{align})}=\frac{1}{|S_c|}\sum_{n \in S_c}\|x_n - \mathrm{center}_c^{(\mathrm{batch})}\|_2^2
$$
...
- $C_{\mathrm{valid}}$ 表示当前 batch 中样本数大于 1 的有效类别集合
