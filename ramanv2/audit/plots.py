...
from ramanv2.audit.reports import read_delta
...
def plot_prefix_group(
    ...
    for curve_index, subplot in enumerate(axes):
        ...
        for index, (name, items) in enumerate(curves.items()):
            ...
            if curve_index == 0:
                subplot.plot(axis, values, **plot_kwargs)
            else:
                plot_curve_outside_bad_bands(
                    subplot,
                    axis,
                    values,
                    bad_bands,
