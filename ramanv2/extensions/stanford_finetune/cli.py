...
def run_command(args: argparse.Namespace) -> int:
    ...
    from ramanv2.extensions.stanford_finetune.config import (
        PREPARE_CONFIG,
        PRETRAIN_CONFIG,
        TRANSFER_CONFIG,
    )
    ...
    if args.command == "prepare":
        ...
        from ramanv2.extensions.stanford_finetune.dataset import prepare_stanford_dataset
        from ramanv2.extensions.stanford_finetune.pretrain import build_pretrain_config

        config = build_pretrain_config(Config())
        print(prepare_stanford_dataset(PREPARE_CONFIG, config.input))
        return 0
    if args.command == "pretrain":
        from ramanv2.extensions.stanford_finetune.pretrain import run_pretrain

        print(run_pretrain(PRETRAIN_CONFIG))
        return 0
    if args.command == "transfer":
        from ramanv2.extensions.stanford_finetune.transfer import run_transfer
