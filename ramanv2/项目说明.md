...
├─ test_transfer.py                  # 测试菌 *t 副本和迁移清单构建
...
```text
ramanv2/audit/
├─ cli.py                            # 连续审核、人工平移与审图 CLI
├─ workflow.py                       # Stage1/自动平移/Stage2/Stage3 主流程
├─ workspace.py                      # 临时运行池、候选移动和提交前路径管理
├─ test_sync.py                      # 测试菌运行池复制、回写和 *t 重建编排
├─ plots.py                          # 同前缀预览与单文件夹平移对照图
├─ preprocess.py                     # 与训练一致的审核预处理
├─ raw_quality.py                    # Stage 1 原始质量审核
├─ similarity.py                     # Stage 2、Stage 3 近邻相似性审核
├─ records.py / reports.py           # 审核记录、CSV 与 delta 日志
...
└─ <数据集名>/
   ...
   ├─ init.npz                       # init 的压缩归档
   ├─ train/                         # 训练用清洗后数据
   ├─ init_test/                     # 独立测试原始来源（raman.data test 使用）
   ├─ test/                          # 预处理后的独立测试集
   ├─ audit_runs/                    # 连续审核各阶段的带时间戳报告与复核图
   ├─ delete/                        # 连续审核移出的原始文件，按 stage 分目录
   ├─ fig_init/                      # 同前缀审图与审图状态
   ├─ delta.txt / delta_log.txt      # 自动平移累计量与操作记录
...
- 代码标识符使用英文 `snake_case`；中文只用于注释和文档。
...
python -m ramanv2 audit clean --dataset <profile>     # 在临时运行池执行三阶段审核与自动平移
python -m ramanv2 audit plot-prefix --dataset <profile> # 输出同属同前缀审核总览图
python -m ramanv2 audit plot-shift --dataset <profile> --folder <Genus/Folder> # 绘制平移对照图
python -m ramanv2 audit manual-shift --dataset <profile> --folder <Genus/Folder> --delta <cm-1>
```
...
不同数据集的目录名、别名和局部宇宙射线参数在 `ramanv2/data/profiles.py` 里维护。
...
- `init_test/`：独立测试原始来源；
- `test/`：预处理后的独立测试集；
- `fig_init/`：`audit plot-prefix`、`audit plot-shift` 的图像与 delta 审图状态
- `fig_train/`：训练集均值谱图与高层级均值谱图
- `audit_runs/`：连续审核的分阶段报告、候选表和复核图
- `delete/`：连续审核移出的原始 `.arc_data` 文件

### 4.4 原始库审核与人工移除

审核按 Stage 1（原始坏谱）→ 自动平移 → Stage 2（同属同前缀全量近邻）→ Stage 3（单批次近邻）顺序执行。
...
Stage 1 后，在 1002 cm⁻¹ 锚点附近估计各文件夹的峰位，并在满足质量、残余范围和累计平移限制时写回波数列，累计量保存在数据集根目录的 `delta.txt`，操作历史保存在 `delta_log.txt`

#### Stage 2 / Stage 3：近邻相似性

Stage 2 在同属同前缀的全量池中判断单谱是否偏离本类；

Stage 3 使用相同指标，但只在当前采集批次（小文件夹）内部比较
...
- `local_area`：目标谱相对近邻中位参考谱的平滑残差超额绝对面积
...
局部残差面积以 5 点移动平均后的绝对残差计算：

$$
\operatorname{local\_area}_i=
\sum_{\lambda=1}^{L}\max\left(
\left|\operatorname{MA}_5\left(x^{(i)}-\operatorname{ref}_i\right)(\lambda)\right|-0.30,\,0
\right)
$$
...
令 $c$、$e$、$a$ 分别表示 `neighbor_corr`、`rmse`、`local_area`。强离群 `candidate` 必须同时满足：
...
满足强离群条件的谱会标记为 `candidate` 并由对应阶段移入 `delete/stage2` 或 `delete/stage3`；

满足局部异常条件的谱标记为 `local_review`，仅写入报告而不移动
...
└─ level_2/
   └─ level_2_<parent_idx>/
      ...
      └─ run_<时间戳>/
         ...
         └─ level_2_<parent_idx>_model.se_stats.pt
```

- `shared_config.yaml` 保存同一实验根内必须一致的输入和预处理配置

- `run_*` 内的 `model_config.yaml` 保存该模型本次训练可独立调整的训练、损失和结构配置
- `resolved_config.yaml` 是两者合成后的完整快照
...
- `CURRENT_TRAIN_LEVEL` 表示“这次训练实际要训练的那一层”
...
1. 从 `EXP_DIR` 加载 `shared_config.yaml`、`hierarchy_meta.json`，并按实际 run 读取 `model_config.yaml`
...
`single_model` 只加载指定 `run_*` 目录中的一个模型，适合训练后立刻检查单个 run 的表现

`level_only` 只使用目标层模型，按验证样本的真实父类分发到对应 parent 子模型，适合判断某一层本身的分类能力

`cascade` 从顶层到目标层逐级使用模型预测结果向下分发，适合模拟真实推理链路

### 8.4 评估输出

`evaluate.py` 的输出目录取决于所选模式：

```text
单模型：       <run_*>/val_result/
单层多模型：   <EXP_DIR>/<level_x>/level_only_result/val_result/
多层级联：     <EXP_DIR>/<level_x>/cascade_result/val_result/
```
...
```text
单模型：       <run_*>/baseline_val_result/
单层多模型：   <EXP_DIR>/<level_x>/level_only_result/baseline_val_result/
多层级联：     <EXP_DIR>/<level_x>/cascade_result/baseline_val_result/
```
...
```powershell
python -m raman.infer test --exp-dir output/<dataset>/<experiment> --level level_1
```

如果不想用命令行，可以直接改根目录 `infer_test.py` 顶部配置后运行：

```python
EXP_DIR = "output/<dataset>/<experiment>"
LEVEL = "level_1"
FOLDER = None
```

`infer_test.py` 会从实验配置读取其余输入参数。
...
默认 `HEATMAP_SEPARATE_CLASS_PLOTS = False`，会输出一张类别汇总热图

改为 `True` 后，会改为按类别分别输出 `band_importance_heatmap__<类别路径>.png`，完整逐点 CSV 仍然保留
