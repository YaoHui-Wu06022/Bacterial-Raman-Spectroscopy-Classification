...
python -m raman.data test <数据集名或profile>       # 从init_test直接清洗并生成test
python -m raman.data plot <数据集名或profile>       # 从已有train独立生成fig_train审图

python -m raman.audit clean                           # 执行三阶段审核与自动平移
python -m raman.audit preview --dataset cos           # 输出同属同前缀中位谱
python -m raman.audit plot-shift --dataset cos --folder <Genus/Folder>  # 复核已平移文件夹
python -m raman.audit rollback-shift --dataset cos    # 撤销当前累计平移
```
...
训练集审图参数单独放在 `raman/data/plot.py` 的 `TrainPlotConfig` 中

`norm_method` 未指定时，会跟随当前运行配置中的标准化方法
...
- `init_test/`：独立测试原始来源；
- `test/`：预处理后的独立测试集；
- `fig_init/`：`raman.audit preview`、`plot-shift` 输出和 delta 审图状态
...
`raman.data train` 只负责生成 `train/` 和日志，成功重建后会清空旧 `fig_train/`
