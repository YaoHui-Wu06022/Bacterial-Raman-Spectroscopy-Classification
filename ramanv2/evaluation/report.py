...
def _write_confusion_matrix(path: Path, matrix: np.ndarray, class_names: Sequence[str]) -> None:
    ...
    annotation_size, tick_size = resolve_confusion_matrix_font_sizes(len(display_names))
    figure = plt.figure(figsize=resolve_confusion_matrix_figsize(display_names))
    axis = sns.heatmap(
        normalized,
        cmap="Blues",
    ...
    figure.subplots_adjust(
        left=max(
            figure.subplotpars.left,
            resolve_confusion_matrix_left_margin(display_names),
        )
    )
    if colorbar is not None:
        heatmap_position = axis.get_position()
